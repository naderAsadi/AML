...
METHODS = ['icarl', 'er', 'er_ace', 'er_aml_triplet', 'er_aml', 'iid', 'iid++', 'icarl_mask', 'icarl_triplet', 'er_multihead', 'der']
...
# ER-AML SupCon hparams
parser.add_argument('--supcon_temperature', type=float, default=0.15)
parser.add_argument('--use_all_negatives', type=int, default=0)
...
if args.wandb_log != 'off':
    import wandb
    # wandb.login(key='')
    wandb.init(project=args.wandb_project, entity=args.wandb_entity, name=args.exp_name, config=args)
...
def eval_agent(agent, loader, task, mode='valid'):
    ...
    for task_t in range(task + 1):
        ...
        logging_per_task(wandb, LOG, mode, 'acc', task, task_t,
            np.round(np.mean(LOG_temp['acc']), 3))

    print(LOG[mode]['acc'][:task+1, :task+1])
    print(f"_anytime_acc_avg: {LOG[mode]['acc'][0:task + 1, task].mean()}")
...
model = ResNet18(
        ...
        dist_linear=args.method in ['er_aml_triplet', 'er_ace', 'er_aml']
        )
...
for task in range(args.n_tasks):
    ...
    for i, (data, target) in enumerate(train_loader):
        ...
        if args.method == 'er_aml_triplet' or 'er_aml':
